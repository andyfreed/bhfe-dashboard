...
export default function AnalyticsPage() {
  ...
  const loadConfig = async () => {
    ...
    const { data: settingsData } = await supabase
      .from('app_settings')
      .select('value')
      .eq('key', 'google_search_console_site_url')
      .single()

    if (settingsData) {
      setConfig(prev => ({ ...prev, siteUrl: settingsData.value || 'https://www.bhfe.com' }))
    }
  }
      ...
      if (config.siteUrl) {
        try {
          ...
          if (scResponse.ok) {
            ...
            setError(errorData.error || 'Failed to fetch Search Console data')
          }
        } catch (err) {
        ...
        setError('Search Console Site URL is required. Please configure it in settings.')
        return
      }

      if (Object.keys(results).length === 0) {
        setError('No data available. Please configure Search Console Site URL in settings.')
        return
      }
